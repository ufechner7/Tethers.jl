# adding 1 AOA
# adding symmetrical actuation by changing l3 (power/depower) between (0,1)
# TODO  read depower uwe, add his code, make work
# add alpha0???
using Timers
tic()
...
@with_kw mutable struct Settings2 @deftype Float64
    ...
    duration::Float64 = 5                                # simulation duration [s]
    dt = 0.05                                              # time step [s]
...
function model(s, pos, vel)
    ...
    conn = vcat(s.set.conn, [(6+i, 6+i+1) for i in 0:(s.set.segments-2)]...)
    conn = vcat(conn, [(6+s.set.segments-1, 1)]) # final connection last tether point to bridle point
