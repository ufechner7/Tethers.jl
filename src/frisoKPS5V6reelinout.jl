...
F2::Vector{Float64} = [0.0, 0.0,  87]
F3::Vector{Float64} = [0.0, 0.0, 87]
F4::Vector{Float64} = [0.0, 0,  87]
F5::Vector{Float64} = [0.0,  0, 87]
V_RO::Float64 = 0.5                          # reel-out velocity                [m/s]
tethersegments::Int64 = 8
...
for i in 1:segments  
    ...
    local eqs = [
       ...
       c_spring[i]        ~ (k_segments[i]/rest_lengths[i]) * (0.1 + 0.9*(len[i] > rest_lengths[i])),
       spring_force[:, i] ~ (c_spring[i]*(len[i] - rest_lengths[i]) + damping * spring_vel[i]) * unit_vector[:, i],
