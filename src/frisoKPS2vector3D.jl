...
F2::Vector{Float64} = [0.0, 0.0,  13.0]
F3::Vector{Float64} = [0.0, 0.0, 13.0]
segments::Int64 = 3   
...
@variables acc(t)[1:3, 1:segments]
...
for i in 1:length(Z)
    ...
    #println(z)
    #println(y)
    lines,sc = plot_kite(y,z,ylim,zlim,lines,sc)
    #plt.tight_layout()
    plt.pause(0.01)
