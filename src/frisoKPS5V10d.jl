...
# -----------------------------
    ...
    simple_sys, pos, vel, conn, e_x, e_y, e_z
end
# -----------------------------
# Simulation Function
# -----------------------------
...
function main()
    se = Settings()
    simple_sys, pos, vel, conn, e_x, e_y, e_z = model(se)
    sol, elapsed_time = simulate(se, simple_sys, pos, vel)
    for i in 1:length(sol.t)
        println("Reference frame at t=", sol.t[i], ":")
        println("e_x: ", sol[e_x, i])
        println("e_y: ", sol[e_y, i])
        println("e_z: ", sol[e_z, i])
        println()
    end
