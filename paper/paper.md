...
`Tethers.jl` addresses this in two ways. First, it is a tutorial: a graded series of
...
The package provides fourteen Julia examples and ten matching Python versions, reachable from an
interactive menu; the `TetherComponents` submodule with the `Point3D`, `Tether`, `FixedEnd`
and `FreeEnd` components and the `TetherSettings` parameter struct; helper functions to copy
the examples and launcher scripts into a user's own project; and a test suite that compares
the Julia and Python results and checks the component against analytic results for the
steady state, the drag and the catenary shape. Documentation, including the full derivation
and all examples, is published online.
