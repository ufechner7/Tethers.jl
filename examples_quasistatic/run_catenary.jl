...
state_vec, tether_pos, Ft_ground, Ft_kite, p0 =  tetherQuasiStatic(state_vec, kite_pos, kite_vel, wind_vel, tether_length, settings)

x_qs = vec(sqrt.(tether_pos[1,:].^2 + tether_pos[2,:].^2))
y_qs = vec(tether_pos[3,:])

# Get catenary curve
x_cat, y_cat = getAnalyticCatenary("test/data/input_analytic_catenary.mat")
