...
without and a run with state events at each segment's taut/slack transition.
This mirrors Tether_06c.jl. Tether_06.py avoids state events entirely by smoothly
blending the taut/slack transition; here we use the same hard switch as the Julia version
instead, and let the integrator place an exact step boundary at each crossing so the
Newton iteration does not have to step blindly over the discontinuity.
...
formula and a sparse Newton solve. Event detection is CasADi's `zero` entry, which takes
the same role as Assimulo's `state_events`.
...
def calc_c_spring(norm, l_seg, c_spring0):
    ...
    return c_spring0 / (1 + REL_COMPRESSION_STIFFNESS) * (REL_COMPRESSION_STIFFNESS + taut)
...
def build_model():
    """ The tether as one CasADi expression graph. Returns the symbolic time `t`, the
        state vector `y`, its derivative `ydot`, the event indicators and `y0`.

        The event threshold is the fixed *initial* segment length L0/SEGMENTS, not the
        growing rest length (L0 + V_RO*t)/SEGMENTS used in the spring force, matching
        Tether_06c.jl - so each segment's event fires once, shortly after start, as its
        actual separation outgrows the initial threshold under reel-out. No state has to
        change at the crossing: the hard switch is recomputed from the current position
        on every call. Registering the event is what makes the integrator place an exact
        step boundary at the crossing instead of stepping over it. """
    n = SEGMENTS
    t = ca.SX.sym('t')
    ...
    events = ca.vertcat(*[ca.norm_2(pos[k+1] - pos[k]) - L0/n for k in range(n)])

    y0 = np.zeros(3 * (2*n + 1))
    for k in range(n + 1):
        l = -k * L0 / n
        y0[3*pos_block(k):3*pos_block(k) + 3] = [math.sin(ALPHA0) * l, 0.0, math.cos(ALPHA0) * l]
    return t, y, ca.vertcat(*ydot), events, y0
...
def run_example(callbacks=True):
    """ Run one simulation, with or without state events, and return
        (t, pos_z, vel_z, full_state) of the lowest mass. """
    t, y, ydot, events, y0 = build_model()
    ...
    sim = ca.integrator('sim', 'cvodes', dae, 0.0, t_sol[1:],
    ...
    vel_z_ix = pos_z_ix + 3
    return t_sol, y_sol[:, pos_z_ix], y_sol[:, vel_z_ix], y_sol
